...
`HeaderLinkBar` renders a semantic `<header>` with an editable brand link and a repeatable horizontal navigation list. Each link accepts a label and either a normal URL or an in-page anchor such as `#work`. On narrow screens the link row scrolls horizontally instead of wrapping into a second navigation bar. The block supports the shared `paper`, `black`, and `lime` themes.
...
Both blocks are available in the Navigation category and are allowed inside nested layout containers. They use `--block-bg` and `--block-ink`, which inherit the site’s configured theme colors, and their layout is mobile-first with a desktop breakpoint at 700px.
