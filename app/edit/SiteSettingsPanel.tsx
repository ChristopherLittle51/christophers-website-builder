...
import { MediaUpload } from '@/lib/site-builder/shared';
...
export default function SiteSettingsPanel({ onClose, onConvert, beforeSave, onSettingsChange }: { onSettingsChange: (settings: SiteSettings) => void; onClose: () => void; onConvert: () => Promise<void>; beforeSave: () => Promise<void> }) {
  ...
  return <dialog ref={dialog} className="site-settings-dialog" aria-labelledby="site-settings-title" onCancel={onClose} onClose={onClose}>
    ...
    <fieldset disabled={!loaded || busy}><legend>SEO and social defaults</legend>
      <p>Published pages automatically receive canonical URLs, search metadata, Open Graph and X/Twitter cards. Page settings override these defaults. Leave page sharing fields blank to inherit.</p>
      <label>Public site URL<input type="url" placeholder="Automatic from the current domain" value={settings.seo.siteUrl} onChange={event => patch({ seo: { ...settings.seo, siteUrl: event.target.value } })} /></label>
      <p>Use your preferred domain, including https://. A hosting-configured SITE_URL takes precedence.</p>
      <label>Site name<input value={settings.seo.siteName} onChange={event => patch({ seo: { ...settings.seo, siteName: event.target.value } })} /></label>
      <label>Default search and social description<textarea value={settings.seo.description} onChange={event => patch({ seo: { ...settings.seo, description: event.target.value } })} /></label>
      <MediaUpload kind="image" label="Default social preview image" value={settings.seo.socialImage} readOnly={!loaded || busy} onChange={socialImage => patch({ seo: { ...settings.seo, socialImage } })} />
      <label>Default social image description<input value={settings.seo.socialImageAlt} onChange={event => patch({ seo: { ...settings.seo, socialImageAlt: event.target.value } })} /></label>
      <p><a href="/robots.txt" target="_blank" rel="noreferrer">View robots.txt</a> · <a href="/sitemap.xml" target="_blank" rel="noreferrer">View sitemap.xml</a></p>
      <p>The sitemap includes published, indexable pages with their own canonical URL. Page settings can exclude a page or point its canonical URL to the original version elsewhere.</p>
    </fieldset>
    <fieldset disabled={!loaded || busy}><legend>Crawlers and Markdown</legend>
      <p>Robots rules express crawler preferences; they do not restrict access. Disabling search crawling also marks pages noindex and empties the sitemap. Crawlers must revisit a page to see changed indexing metadata. Search engines receive HTML by default.</p>
