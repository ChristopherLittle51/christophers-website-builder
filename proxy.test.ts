...
test('preserves implicit AI negotiation so the content route can enforce aiMarkdown', async () => {
  const previousFetch = globalThis.fetch;
  const previousOrigin = process.env.RENDER_ORIGIN;
  process.env.RENDER_ORIGIN = 'https://renderer.test';
  let received: Request | undefined;
  globalThis.fetch = async (input, init) => {
    received = new Request(input, init);
    return new Response('<main>Published HTML</main>', { headers: { 'content-type': 'text/html' } });
  };
  try {
    const response = await proxy(new NextRequest('https://site.test/work', { headers: { 'user-agent': 'GPTBot/1.0' } }));
    assert.equal(received?.headers.get('accept'), '*/*');
