...
export const compositionComponents: Record<string, any> = {
  LayoutContainer: {
        ...
        render: ({ columns, ratio, gap, padding, verticalAlign, theme, first: First, second: Second, third: Third, fourth: Fourth }) => <section className={`builder-container builder-container--${columns} builder-container--${ratio} builder-container--gap-${gap} builder-container--pad-${padding} builder-container--align-${verticalAlign} builder-theme--${theme}`}><div className="builder-container__cell"><StableSlot render={First} className="builder-container__dropzone" minEmptyHeight={220} /></div>{columns !== 'one' ? <div className="builder-container__cell"><StableSlot render={Second} className="builder-container__dropzone" minEmptyHeight={220} /></div> : null}{columns === 'three' || columns === 'four' ? <div className="builder-container__cell"><StableSlot render={Third} className="builder-container__dropzone" minEmptyHeight={220} /></div> : null}{columns === 'four' ? <div className="builder-container__cell"><StableSlot render={Fourth} className="builder-container__dropzone" minEmptyHeight={220} /></div> : null}</section>,
  },
  FlexRow: {
        label: 'Flex row',
        fields: {
  ...
  InsetContainer: {
        ...
        render: ({ width, padding, align, theme, content: Content }) => <section className={`builder-inset builder-inset--width-${width} builder-inset--pad-${padding} builder-inset--${align} builder-theme--${theme}`}><div>{Content ? <StableSlot render={Content} /> : null}</div></section>,
  },
  AspectRatio: {
        ...
        render: ({ ratio, overflow, theme, content: Content }) => <div className={`builder-aspect-frame builder-aspect-frame--${ratio} builder-aspect-frame--overflow-${overflow} builder-theme--${theme}`}><div>{Content ? <StableSlot render={Content} /> : null}</div></div>,
  },
  MediaText: {
        ...
        render: ({ image, crop, alt, side, ratio, content: Content, theme }) => <section className={`builder-media-text builder-media-text--${side} builder-media-text--ratio-${ratio} builder-theme--${theme}`}><img src={image} alt={alt || ''} style={imagePosition(crop)} /><div>{Content ? <StableSlot render={Content} /> : null}</div></section>,
  },
};
